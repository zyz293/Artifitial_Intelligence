...
class HMM:
    ...
    def label(self, data):
        ...
        # self.priors = {'drawing': 0.6, 'text': 0.4}
        # self.transitions = {'drawing': {'drawing': 0.5, 'text': 0.125}, 'text': {'drawing': 0.25, 'text': 0.375}}
        # self.emissions = {'drawing': {'length': [0.6, 0.15]}, 'text': {'length': [0.05, 0.35]}}
        ...
        i = data[0]
        if i['length'] == 1:
            prob_draw *= self.priors['drawing'] * self.emissions['drawing']['length'][1]
            prob_text *= self.priors['text'] * self.emissions['text']['length'][1]
        elif i['length'] == 0:
            prob_draw *= self.priors['drawing'] * self.emissions['drawing']['length'][0]
            prob_text *= self.priors['text'] * self.emissions['text']['length'][0]
        if prob_draw > prob_text:
            path.append('drawing')
        else:
            path.append('text')
        for k in data[1:]:
            if k['length'] == 1:
                if prob_draw * self.transitions['drawing']['drawing'] > prob_text * \
                        self.transitions['text']['drawing']:
                    prob_draw *= self.transitions['drawing']['drawing'] * self.emissions['drawing']['length'][1]
                else:
                    prob_draw = prob_text * self.transitions['text']['drawing'] * self.emissions['drawing']['length'][1]
                if prob_draw * self.transitions['drawing']['text'] > prob_text * self.transitions['text']['text']:
                    prob_text = prob_draw * self.transitions['drawing']['text'] * self.emissions['text']['length'][1]
                else:
                    prob_text *= self.transitions['text']['text'] * self.emissions['text']['length'][1]
            elif k['length'] == 0:
                if prob_draw * self.transitions['drawing']['drawing'] > prob_text * \
                        self.transitions['text']['drawing']:
                    prob_draw *= self.transitions['drawing']['drawing'] * self.emissions['drawing']['length'][0]
                else:
                    prob_draw = prob_text * self.transitions['text']['drawing'] * self.emissions['drawing']['length'][0]
                if prob_draw * self.transitions['drawing']['text'] > prob_text * self.transitions['text']['text']:
                    prob_text = prob_draw * self.transitions['drawing']['text'] * self.emissions['text']['length'][0]
                else:
                    prob_text *= self.transitions['text']['text'] * self.emissions['text']['length'][0]
            if prob_draw > prob_text:
                path.append('drawing')
            else:
                path.append('text')
...
class StrokeLabeler:
    ...
    def confusioninput(self, trainingDir):
        for fFileObj in os.walk(trainingDir):
            lFileList = fFileObj[2]
            break
        goodList = []
        for x in lFileList:
        # for x in lFileList[0:5]:
            if not x.startswith('.'):
                goodList.append(x)
        ...
        classifications = []
        for i in allStrokes:
            temp = self.labelStrokes(i)
            classifications.append(temp)
        return self.confusion(allLabels, classifications)

    def confusion(self, trueLabels, classifications):
        matrix = {'drawing': {'drawing': 0, 'text': 0}, 'text': {'drawing': 0, 'text': 0}}
        for i in range(len(trueLabels)):
            true = trueLabels[i]
            classify = classifications[i]
            for j in range(len(true)):
                if true[j] == 'drawing':
                    if classify[j] == 'drawing':
                        matrix['drawing']['drawing'] += 1
                    elif classify[j] == 'text':
                        matrix['drawing']['text'] += 1
                elif true[j] == 'text':
                    if classify[j] == 'drawing':
                        matrix['text']['drawing'] += 1
                    elif classify[j] == 'text':
                        matrix['text']['text'] += 1
        return matrix
