...
class HMM:
    ...
    def label(self, data):
        ...
        prob_text = 1
        i = data[0]
        prob_draw *= self.priors['drawing'] * self.getEmissionProb('drawing', i)
        prob_text *= self.priors['text'] * self.getEmissionProb('text', i)
        ...
        for k in data[1:]:
            if prob_draw * self.transitions['drawing']['drawing'] > prob_text * \
                ...
                prob_draw *= self.transitions['drawing']['drawing'] * self.getEmissionProb('drawing', k)
            else:
                prob_draw = prob_text * self.transitions['text']['drawing'] * self.getEmissionProb('drawing', k)
            if prob_draw * self.transitions['drawing']['text'] > prob_text * self.transitions['text']['text']:
                prob_text = prob_draw * self.transitions['drawing']['text'] * self.getEmissionProb('text', k)
            else:
                prob_text *= self.transitions['text']['text'] * self.getEmissionProb('text', k)
            if prob_draw > prob_text:
                path.append('drawing')
            else:
                path.append('text')
...
class StrokeLabeler:
    def __init__(self):
        ...
        self.featureNames = ['length', 'speed', 'curvature']
        self.contOrDisc = {'length': DISCRETE, 'speed': DISCRETE, 'curvature': DISCRETE}
        self.numFVals = { 'length': 2, 'speed': 2, 'curvature': 2}
    ...
    def featurefy( self, strokes ):
        ...
        for s in strokes:
            ...
            l = s.length()
            time = s.points[0][2]
            speed = float(l) / (float(time) - float(timeprev))
            # print speed
            timeprev = time
            if l < 300:
                d['length'] = 0
