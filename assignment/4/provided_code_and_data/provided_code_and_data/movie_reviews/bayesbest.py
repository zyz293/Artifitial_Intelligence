...
class Bayes_Classifier:
    ...
    def trainCV(self, i, filelist, Precision, Recall, Fvalue):
        ...
        # print len(test)
        # print 'test: ' ,test
        # test = filelist[(i*1386):(i*1386+200)]
            ...
            if txt.split('-')[1] == '1':
                ...
                for word in text:
                    if re.match('[a-zA-Z0-9]', str(word)) is not None:
                        ...
                        if stemword in CV_Neg:
                            CV_Neg[str(stemword)] += 1
                        else:
                            CV_Neg[str(stemword)] = 1
            elif txt.split('-')[1] == '5':
                path = 'movies_reviews/%s' % str(txt)
                ...
                for word in text:
                    if re.match('[a-zA-Z0-9]', str(word)) is not None:
                        ...
                        # else:
                           #  CV_Pos[str(word)] = 1
                        stemword = stemmer.stem(word)
                        if stemword in CV_Pos:
                            CV_Pos[str(stemword)] += 1
                        else:
                            CV_Pos[str(stemword)] = 1
            # if txt.split('-')[1] == '1':
            #     path = 'movies_reviews/%s' % str(txt)
             #    st = self.loadFile(path)
              #   text = self.tokenize(st)
               #  for k in range(len(text)-1):
                #     bigrams = text[k] + ' ' + text[k+1]
                 #    if bigrams in CV_Neg:
                  #       CV_Neg[str(bigrams)] += 1
                   #  else:
                    #     CV_Neg[str(bigrams)] = 1
            # elif txt.split('-')[1] == '5':
             #    path = 'movies_reviews/%s' % str(txt)
              #   st = self.loadFile(path)
               #  text = self.tokenize(st)
                # for j in range(len(text)-1):
                 #    bigrams = text[j] + ' ' + text[j+1]
                  #   if bigrams in CV_Pos:
                   #      CV_Pos[str(bigrams)] += 1
                    # else:
                     #    CV_Pos[str(bigrams)] = 1
    ...
    def classifyCV(self, test, Precision, Recall, Fvalue):
        ...
        for txt in test:
            ...
            for word in testtext:
                if re.match('[a-zA-Z0-9]', str(word)) is not None:
                    # if word in CV_Pos:
                       #  P_pos = P_pos + math.log((float(CV_Pos[str(word)] + 1.0) / float(sum(CV_Pos.values()))))
                    # else:
                       #  P_pos = P_pos + math.log(1.0 / float(sum(CV_Pos.values())))
                    # if word in CV_Neg:
                      #   P_neg = P_neg + math.log((float(CV_Neg[str(word)] + 1.0) / float(sum(CV_Neg.values()))))
                    # else:
                       #  P_neg = P_neg + math.log(1.0 / float(sum(CV_Neg.values())))
                    stemword = stemmer.stem(word)
                    if stemword in CV_Pos:
                        P_pos = P_pos + math.log((float(CV_Pos[str(stemword)] + 1.0) / float(sum(CV_Pos.values()))))
                    else:
                        P_pos = P_pos + math.log(1.0 / float(sum(CV_Pos.values())))
                    if stemword in CV_Neg:
                        P_neg = P_neg + math.log((float(CV_Neg[str(stemword)] + 1.0) / float(sum(CV_Neg.values()))))
                    else:
                        P_neg = P_neg + math.log(1.0 / float(sum(CV_Neg.values())))
            # print P_pos, P_neg
            # for n in range(len(testtext) - 1):
             #    bigrams = testtext[n] + ' ' + testtext[n+1]
              #   # print bigrams
               #  if bigrams in CV_Pos:
                #     P_pos = P_pos + math.log((float(CV_Pos[str(bigrams)] + 1.0) / float(sum(CV_Pos.values()))))
                # else:
                 #    P_pos = P_pos + math.log(1.0 / float(sum(CV_Pos.values())))
             #    if bigrams in CV_Neg:
              #       P_neg = P_neg + math.log((float(CV_Neg[str(bigrams)] + 1.0) / float(sum(CV_Neg.values()))))
               #  else:
                #     P_neg = P_neg + math.log(1.0 / float(sum(CV_Neg.values())))
            ...
            if classfication == flag and flag == 'positive':
                pos_right += 1
            elif classfication == flag and flag == 'negative':
                neg_right += 1
        print 'pos_right: ' ,pos_right
        print 'pos_counter: ' ,pos_counter
        print 'pos_review: ' ,pos_review
        precision = float(float(pos_right) / float(pos_counter))
        print 'precision: ' ,precision
        recall = float(float(pos_right) / float(pos_review))
        print 'recall: ' ,recall
